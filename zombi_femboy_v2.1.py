...
comeco = input("bem vindo ao apocalipse\nvoce gostaria de jogar? (sim ou não)\n").lower()
if comeco == "sim" or comeco == "s":
    os.system('cls')
...
while vida > 0 and vida_zumbi1 > 0:
    acao = input(f"\n{nome} ataca: oque voce vai fazer?(atacar ou acariciar)\n")

    if acao == "atacar":
        vida_zumbi1 -= ataque
        print(f"voce causou {ataque} de dano\na vido do zumbi femboy agora é: {vida_zumbi1}")
...
while incrementa > 10 or incrementa < 0:
    print("Você só pode colocar entre 0 e 10 pontos, tente novamente.")
    time.sleep(3)
    os.system('cls')
    incrementa = int(input("Quantos pontos você quer colocar em ataque? (o resto vai para a vida)\n"))
